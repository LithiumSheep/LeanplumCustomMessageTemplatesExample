...
public class BaseMessageDialog extends Dialog {
  ...
  protected BaseMessageDialog(Activity activity, boolean fullscreen, BaseMessageOptions options,
    ...
    view.setId(com.leanplum.core.R.id.container_view);
  ...
  private CloseButton createCloseButton(Activity context, boolean fullscreen, View parent) {
    ...
    closeButton.setId(com.leanplum.core.R.id.close_button);
  ...
  private RelativeLayout createContainerView(Activity context, boolean fullscreen) {
    ...
    if (!isWeb && !isHtml) {
      ...
      title.setId(com.leanplum.core.R.id.title_view);
      view.addView(title, title.getLayoutParams());

      View button = createAcceptButton(context);
      button.setId(com.leanplum.core.R.id.accept_button);
   ...
   *
   * 4/24/19 - Jesse Li
   * This method was modified from the SDK BaseMessageDialog to
   * 1) Enable scrollbars
   *   - webView.setVerticalScrollBarEnabled(false) -> true
   *   - webView.setHorizontalScrollBarEnabled(false) -> true
   * 2) Remove OnTouchListener that was intercepting MotionEvent.ACTION_MOVE
   * As a result, content can be scrolled by touch in the templates that extend this class
   * (CenterPopup, HTMLTemplate, Interstitial, and WebInterstitial).
   *
